    ...
    productModel.find((err, data) => {
        if (!err) {
            categoryModel.find({ category_status: 1 }, (errors, cat) => {
                if (!errors) return res.render("admin/products/product", { layout: "backend", list: data, cat: cat });
                else return res.render("admin/products/product", { layout: "backend", list: data });
            }).lean();
        } else {
            res.render("admin/products/product", { layout: "backend", list: data });
        }
    }).lean()
}
controller.addProduct = (req, res, next) => {
    ...
    //     for(let val in errors.mapped()){
    //         formError[val] = errors.mapped()[val].msg;
    //     }
    //     return res.send({status:0, formError: formError});
    // }
    const body = req.body;
        ...
        if (!err) {
            ...
            productData.save((err, data) => {
                if (!err) {
                    ...
                    <td><button class="btn btn-info px-2 py-1" onclick="getId('${data._id}')"> <i class="fa fa-edit"></i> </button>
                    <button class="btn btn-danger px-2 py-1" onclick="deleteData('${data._id}')"> <i class="fa fa-trash"></i> </button></td>
                </tr> `;
                    res.send({ status: 1, title: "Done!", message: "Category Added successfully", modal: "success", lastRow: row });
